...
//    }
    ...
    public String southSend() throws IOException {
        HttpResponse response = Curl.send("domainApplicationBici", "Password1", payloadCreator() );
        System.out.println("Response received. Parsing it...");
    ...
    public String northReceive() throws IOException {
        HttpResponse response = Curl.receive("domainApplicationBici", "Password1");
        return Curl.getContentFromHttpResponse(response);
    ...
    public String northFilter() throws IOException, XMLStreamException {
        ...
        HttpResponse response = Curl.receive("domainApplicationBici", "Password1");
        return Curl.filterXMLContentAsString(response, "deviceBiciStation");
    }

    @RequestMapping("/bike/new/{urn}")
    public String newBike(@PathVariable("urn") String name) {
        new Device("Bike1", true);
        return "Done?";
    }
...
//    }
    ...
    private String payloadCreator() {
        String domainApplication = "domainApplicationBici";
        String user = "userBici";
        String deviceStation = "deviceBiciStation"; // "deviceCurlTestStation"; //devicedeviceGatewayCurlTest2
        String sensor = "sensorBici"; // "sensorCurlTest"; //sensorCurlTest2
        String resourceSet = "resourceSetCurlTest";
        String resource = "resourceSetBici.resourceBici"; // "resourceCurlTest"; //resourceresourceCurlTest2
